...
scaling_factor_listH_L=[scaling_factor_spp(:,1),scaling_factor_spp(:,3)]; %dorsal and ventral of left hindwing
scaling_factor_listH_R=[scaling_factor_spp(:,1),scaling_factor_spp(:,3)]; %dorsal and ventral of right hindwing

%Flipping the right hindwing to lefthand side
...
having_tail=(having_tail_L+having_tail_R)>0; %List specimen with extended wing regions
...
firstColLastRow_allLen(round(firstColLastRow_allLen,1)<length_threshold)=NaN; %replace small value with NaN, and neglect those with length less than 5 px (~0.1mm)
firstColLastRow_allCur(round(firstColLastRow_allCur,2)==0)=NaN; %replace 0 with NaN
